...
namespace NaiveAPI.UITK
{
    ...
    {
        public static readonly List<Type> ActiveTypes = new List<Type>();
        static TypeReader()
        {
            foreach(var asm in AppDomain.CurrentDomain
                .GetAssemblies()
                .Where(asm => { return !UIElementExtensionResource.Get.IgnoreAssemblyName.Contains(asm.GetName().Name); }))
            {
