...
{
    [System.Serializable]
    public sealed class RSMargin : RSStyleComponent<RSMargin>
    {
        ...
        public static readonly RSMargin Clear = new RSMargin() { any = RSLength.Pixel(0) };
        #endregion

        public override RSStyleFlag StyleFlag => RSStyleFlag.Margin;
        ...
        {
            if (GetFlag(F_Left))   style.marginLeft   = m_Left;
            if (GetFlag(F_Top))    style.marginTop    = m_Top;
            if (GetFlag(F_Right))  style.marginRight  = m_Right;
            if (GetFlag(F_Bottom)) style.marginBottom = m_Bottom;
        }
        public override void LoadFrom(RSMargin other)
        {
            m_flag = other.m_flag;
            m_Left = other.m_Left;
            m_Top = other.m_Top;
        ...
        {
            m_flag   = -1;
            m_Left   = style.marginLeft.value;
            m_Top    = style.marginTop.value;
            m_Right  = style.marginRight.value;
            m_Bottom = style.marginBottom.value;
        }
        public override void LoadFromLerp(RSMargin begin, RSMargin end, float rate)
        {
        ...
        public override void LoadFromIfUnset(RSMargin other)
        {
        ...
        public RSMargin() { }
        public RSMargin(float value, LengthUnit unit = LengthUnit.Pixel)
        {
            any = new RSLength(unit, value);
        }
    }
}
