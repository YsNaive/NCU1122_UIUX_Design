        ...
        where DType : struct
    {
        ...
        public MultipleNumberDrawer()
        {
            ...
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                fields[i].labelElement.style.width = DocStyle.Current.LineHeight;
                fields[i].iconElement.style.display = DisplayStyle.None;
                fields[i].RegisterValueChangedAsEventBase(evt =>
                {
                    SetValueWithoutUpdate(Fields2DataType(fields));
                    evt.StopPropagation();
                });
            }
